...
import { Creator, ReactTag } from '@/types/creator'
...
import { updateCreator } from '@/lib/action'

interface FormEditCreatorProps {
    creator: Creator;
    categories: Category[];
    suggestions: Array<ReactTag>
}

const statuses = ['active', 'pending'];

const FormEditCreator: React.FC<FormEditCreatorProps> = ({ creator, categories, suggestions }) => {
    ...
    useEffect(() => {
        if (creator.tags.length) {
            const newArrayTag = creator.tags.map((tag) => ({ id: tag.name, text: tag.name, className: '' }))
            setTags(newArrayTag)
        }
    }, [])
    ...
    const handleSubmit: FormEventHandler<HTMLFormElement> = async (event) => {
        ...
        const result = await updateCreator(creator, formData);
        if (result?.message) {
    ...
    return (
        <form onSubmit={handleSubmit} className="w-1/2 mx-auto">
            ...
            <div>
                ...
                <input id="name" name="name" type="text" placeholder="Name" defaultValue={creator.name}
            ...
            <div className='mt-3'>
                ...
                <select id="category" name="category" defaultValue={creator.category._id}
                    className="block w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
            ...
            <div className='mt-3'>
                ...
                <textarea id="description" name="description" rows={4} defaultValue={creator.description}
            ...
            <div className='mt-3'>
                ...
                <select id="status" name="status" defaultValue={creator.status}
                    className="block w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                    {statuses.map((status) => (
            ...
            <div className='mt-3'>
                ...
                <PickImage reset={false} valueImageSrc={creator.mediaUrl} />
            </div>
...
export default FormEditCreator
